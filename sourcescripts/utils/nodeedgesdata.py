...
def run_joern(filepath: str, verbose: int):
    ...
    filename = imp.external_dir() / filepath
    params = f"filename={filename}"
    command = f"joern --script {script_file} --param '{params}'"
    command = str(imp.external_dir() / "joern-cli" / command)
    if verbose > 2:
        imp.debug(command)
    imp.subprocess_cmd(command, verbose=verbose)
    try:
        shutil.rmtree(imp.external_dir() / "joern-cli" / "workspace" / filename.name)
...
def get_node_edges(filepath: str, verbose=0):
    ...
    filepath = "/home/david/Documents/projects/singularity-sastvd/storage/processed/Dataset/before/53.c"
    """
